...
#Currently only lists names from MarvelAPI
...
#Function to check if character is in database	
def charInDatabase(name):
	namesList = []
...
def resize(width, height, wbox, hbox, pil_image):
	f1 = 1.0*wbox/width
	#first factor for image size
	f2 = 1.0*hbox/height
	#second factor for image size
	factor = min([f1, f2])
	width = int(width*factor)
	height = int(height*factor)
	return pil_image.resize((width, height), Image.ANTIALIAS)

#Image creation functions that convert Marvel API images (PNG) to Tkinter compatible image
...
#Function for returning characters between two specified characters
...
#Function for scrapping Marvel Wikia Issue page for year of publication
def scrapIssue(issueURL):
	try:		
		soup = BeautifulSoup(urlopen(issueURL), "html.parser")
		lines = soup.find_all('a')
		#First instance of a month name in the Wikia page will be followed by the year of publication
		...
		for line in lines:
			...
			for month in months:
				if month in line:
					...
					return year
	#If Mavel Wikia does not have URL for that issue
	except HTTPError:
...
def createSingleCharPage(characterName):
	if charInDatabase(characterName):
		currentChar = singleChar(characterName.title())
		#Creates singleChar object that holds information about that character
		currentChar.getID()
	...
	#Error catching for when user inputs character not in database
	else:
		errorScreen = Toplevel()
		errorScreen.title("Error")
		errorText = tk.Label(errorScreen, text = characterName + " not in database.", bg = "red4", fg = "white")
		errorText.pack()
...
def createDoubleCharPage(firstCharName, secondCharName):
	if charInDatabase(firstCharName) and charInDatabase(secondCharName):
		currentFirstChar = singleChar(firstCharName.title())
		#Creates singleChar object that holds information about the first character
		currentSecondChar = singleChar(secondCharName.title())
		#Creates singleChar object that holds information about the second character
	...
	#Error catching for when user inputs second character not in database
	elif (charInDatabase(firstCharName) is True) and (charInDatabase(secondCharName) is False):
		errorScreen = Toplevel()
		errorScreen.title("Error")
		errorText = tk.Label(errorScreen, text = secondCharName + " not in database.", bg = "red4", fg = "white")
		errorText.pack()
	#Error catching for when user inputs first character that is not in database
	elif (charInDatabase(firstCharName) is False) and (charInDatabase(secondCharName) is True):
		errorScreen = Toplevel()
		errorScreen.title("Error")
		errorText = tk.Label(errorScreen, text = firstCharName + " not in database.", bg = "red4", fg = "white")
		errorText.pack()
	#Error catching for when user inputs both characters not in database	
	else:
		errorScreen = Toplevel()
		errorScreen.title("Error")
		errorText = tk.Label(errorScreen, text = firstCharName + " and " + secondCharName + " not in database.", bg = "red4", fg = "white")
		errorText.pack()

#For creating singleCharPage when clicking character link on the Double (Common Appearances) screen		
...
#For creating doubleCharPage when clinking on character link on Single (Individual character) screen
...
class startScreen(tk.Frame):
	def __init__(self, parent, controller):
		...
		self.char1Text = tk.Label(self.char1Frame, text="View Individual\n Character's Network", bg = "goldenrod1", height=2, wraplength=graphicWidth, font = ('Helvetica', 20), pady = 5, anchor=NW, borderwidth=2, relief=GROOVE)
		self.char1Text.pack()
		...
		self.char2Text = tk.Label(self.char2Frame, text="Find Common Appearances\n Between Two Characters", bg = "goldenrod1", height=2, wraplength=graphicWidth, font = ('Helvetica', 20), pady=5, anchor=NE, borderwidth=2, relief=GROOVE)
	...
	def char1Display(self, event):
		...
		self.submitSingle = Button(self.char1Frame, text="Enter", command=lambda: createSingleCharPage(self.singleCharEntry.get()))
	...
	def char2Display(self, event):
		...
		self.submitDouble = Button(self.char2Frame, text="Enter", command=lambda: createDoubleCharPage(self.doubleCharEntry1.get(), self.doubleCharEntry2.get()))
...
class singleCharScreen(tk.Frame, singleChar):
	def __init__(self, parent, singleChar):
		...
		self.title = tk.Label(self.leftFrame, text=singleChar.name.upper(), anchor=W, wraplength = wbox, font = ('Helvetica', 24), padx=5, pady=5, bg = "goldenrod1", relief=GROOVE)
		...
		self.encounterName1 = tk.Label(self.enCharFrame, text=singleChar.encounterNames[0], bg = "goldenrod1", relief=GROOVE)
		...
		self.encounterName2 = tk.Label(self.enCharFrame, text=singleChar.encounterNames[1], bg = "goldenrod1", relief=GROOVE)
		...
		self.encounterName3 = tk.Label(self.enCharFrame, text=singleChar.encounterNames[2], bg = "goldenrod1", relief=GROOVE)
		...
		self.encounterName4 = tk.Label(self.enCharFrame, text=singleChar.encounterNames[3], bg = "goldenrod1", relief=GROOVE)
		...
		self.encounterName5 = tk.Label(self.enCharFrame, text=singleChar.encounterNames[4], bg = "goldenrod1", relief=GROOVE)
...
#Comparison between two characters screen
class doubleCharScreen(tk.Frame):
	def __init__(self, parent, currentFirstChar, currentSecondChar):
		...
		self.leftTitle = tk.Label(self.leftFrame, text=firstChar.name.upper(), anchor=N, wraplength = wbox, font = ('Helvetica', 24), padx=5, pady=5, bg="goldenrod1", relief=GROOVE)
		...
		self.rightTitle = tk.Label(self.rightFrame, text=secondChar.name.upper(), anchor=N, wraplength = wbox, font = ('Helvetica', 24), padx=5, pady=5, bg="goldenrod1", relief=GROOVE)
